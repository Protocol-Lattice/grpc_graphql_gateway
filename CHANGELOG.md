...
- **Router Defaults**: Hardened router defaults and tightened configuration validation to prevent insecure deployments.
- **Dependencies**: Resolved security audit findings by updating `Cargo.lock`.

### Fixed

- **Testing**: Fixed test flakiness in `test_multiple_configurations` caused by concurrent environment variable modifications.
- **Code Quality**: Fixed `clippy` warnings and compilation errors across the codebase.
...
### Fixed
- **GBP Encoder Reference Tracking** (`gbp.rs`): Fixed a critical bug where columnar-encoded arrays were not registered in the value pool. This caused redundant encoding of identical arrays and could lead to broken back-references in the binary stream, specifically affecting high-repetition data sets.
...
### Added
- **Configuration**: Enabled environment variable interpolation in `router.yaml`. The router now supports `${ENV_VAR}` and `${ENV_VAR:-default}` syntax for dynamic configuration values.
- **Security**: Updated examples to use the `GATEWAY_SECRET` environment variable for subgraph headers instead of hardcoding secrets in configuration files.

## [1.0.2] - 2026-02-20

### Added
- **GBP Tooling**: Added a new `decode_gbp` binary tool (`src/bin/decode_gbp.rs`) to decode and inspect GraphQL Binary Protocol (GBP) payloads directly from the terminal for debugging compression efficiency.

### Fixed
- **Router Authentication**: Fixed an issue where the router failed to send the required `X-Gateway-Secret` authentication header to subgraphs. The router now correctly includes the header in `examples/router.yaml`, resolving 401 Unauthorized errors and empty data responses.

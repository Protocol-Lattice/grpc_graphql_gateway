...
mod tests {
    ...
    fn test_grpc_entity_resolver_builder() {
        ...
        let _resolver = GrpcEntityResolver::builder(pool)
            .register_entity_resolver("Type", EntityResolverMapping {
                service_name: "S".into(),
                method_name: "M".into(),
                key_field: "k".into(),
            })
    ...
    fn test_federation_config_parsing() {
        ...
        let pool = DescriptorPool::decode(descriptor_bytes.as_slice()).expect("Failed to decode descriptor");
        
        let entity_ext = pool.get_extension_by_name("graphql.entity")
    ...
    async fn test_grpc_entity_resolver_resolve_noop() {
        ...
        let message_descriptor = pool.all_messages().next().expect("No messages in descriptor");
        
        let client_pool_inner = crate::grpc_client::GrpcClientPool::new();
        let client = crate::grpc_client::GrpcClient::connect_lazy("http://localhost:50051", true).unwrap();
        client_pool_inner.add("UserService", client);
        
        let mut resolver = GrpcEntityResolver::new(client_pool_inner);
        resolver.register_entity_resolver("User", EntityResolverMapping {
        ...
        representation.insert(async_graphql::Name::new("id"), GqlValue::String("123".into()));
        representation.insert(async_graphql::Name::new("__typename"), GqlValue::String("User".into()));
        
        let result = resolver.resolve_entity(&entity_config, &representation).await;
    ...
    async fn test_grpc_entity_resolver_batch() {
        ...
        let results = resolver.batch_resolve_entities(&entity_config, vec![repr1, repr2]).await;

...
use grpc_graphql_gateway::{
    Gateway, GrpcClient, CacheConfig, SmartTtlManager, SmartTtlConfig,
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let grpc_client = GrpcClient::builder("http://localhost:50051")
        .connect_lazy()?;
    ...
    tokio::spawn({
        ...
        async move {
            ...
            loop {
                ...
                // Cleanup old stats
                smart_ttl.cleanup_old_stats(Duration::from_secs(86400)).await;
            }
        }
    });
